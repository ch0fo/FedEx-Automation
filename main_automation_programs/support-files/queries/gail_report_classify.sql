...
WHERE
  Classify.SHIPMENT_PROCESS_CONTROL.PROCESS_CONTROL_TYPE_DESC = 'ENTRYS'
    and classify.shipment_party.broker_id_cd  in ('ACU', 'AIC',      'AJZ',      'ARG',    'ARW',   'CPI', 'EVZ',          'FLA',     'HYP',                'HYC',    'FDS',     'MMX',   'PMC',   'TES', 'FEC','FON','FEX')
    
    --CHANGE DATE
    AND classify.shipment.entry_dt between {starting} and {ending}
    --CHANGE DATE
    
and CLASSIFY.SHIPMENT_PROCESS_CONTROL.PROCESS_CONTROL_DATA_DESC in ('LVS')
    AND classify.shipment_party.shipment_party_type_cd = 'B' ), impInfo as 
(SELECT
    broker.*,
...
(SELECT
    ...
    classify.shipment_party.Contact_NM      as sContact,
                classify.shipment_party.customer_acct_nbr as      sAccount
